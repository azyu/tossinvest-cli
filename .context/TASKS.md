...
| 2026-06-21 | Latest OpenAPI contract fidelity pass | Re-fetched `https://openapi.tossinvest.com/openapi-docs/latest/openapi.json` (version `1.1.1`) and added local validation for symbol/symbols, date/date-time, same-currency exchange-rate, order amount/type/price invariants, accountSeq int64 range, and holdings nullable JSON preservation. Verified with `cargo fmt --manifest-path rust/Cargo.toml --all`, `cargo clippy --manifest-path rust/Cargo.toml --all-targets -- -D warnings`, `cargo test --manifest-path rust/Cargo.toml` → 97 passed, `cargo build --manifest-path rust/Cargo.toml -p toss-cli --bin toss`, validation smoke for `market exchange-rate --base USD --quote USD`, and help smoke for updated commands. |
