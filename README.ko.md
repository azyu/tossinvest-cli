...
실제 변경 주문 명령은 `--confirm`이 필요합니다. 아래 명령은 template입니다. 모든 placeholder를 바꾸고 confirmed command가 실제 증권 주문을 전송한다는 점을 이해하기 전에는 copy-paste하지 마세요.
...
Text output은 사람이 읽기 위한 출력입니다. Text mode에서 추가 출력을 줄이려면 `--quiet`를 사용하세요. Text mode에서는 command error가 stderr로 출력됩니다. JSON mode에서는 성공과 오류 envelope가 모두 stdout으로 출력됩니다. 실패한 명령은 non-zero로 종료될 수 있으므로 자동화에서는 JSON envelope와 process status를 모두 확인하세요.
...
- 금액 기반 주문은 `--amount`를 사용합니다. Toss docs 기준으로 미국 시장 정규장 케이스로 제한됩니다.
